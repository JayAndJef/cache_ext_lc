...
- **YCSB+LevelDB (primary benchmark)**: `lc-eval/ycsb/reproduce_eval.sh <db>` is the one-command entrypoint — it produces every result file deterministically (`ycsb_eval_results.json` = 5 classical policies + `ml_protect` matched models; `ycsb_eval_ml{10,20,30,40}.json` = one ML-rank oversampling factor each; `ycsb_eval_lru.json` = Linux classic LRU baseline / MGLRU off; `ycsb_eval_mglru.json` = kernel MGLRU baseline / MGLRU on; `ycsb_eval_meta.json` model mapping; `ycsb_eval_manifest.json` provenance). It sequences `run_model_eval.sh` (classical + ml_protect + both Linux baselines, single pass) and `run_ml_sampling_eval.sh` looped per factor (each factor to its own file — checkpointing can't see loader-only args like `--sample_size`, so they'd collide in one file; the orchestrator derives the filename from the factor so they can't drift). `collect_traces.sh <db>` collects `fifo_lc` tracer training data (binary logs to `/mydata/cache_ext_logs/<benchmark>/iter_<N>/`, three `mglru_lc_{access,insertion,eviction}_{unix_ts}.bin` per dir; read with `policies/read_binary_logs.py`). `collect_evict_latency.sh` captures raw per-call eviction-decision latency to `results/evict_latency/<tag>.trace`. **`visualizations/results.ipynb` is the single consumer** of all the result/`.trace` files — there are no summarize/validation scripts. See `lc-eval/ycsb/README.md` for the full pipeline. (The former generic `run.sh` full-sweep was removed; nothing depended on it.)
- **Twitter traces (paper Fig 8, clusters 17/18/24/34/52)**: artifacts via `./download_twitter_dbs.sh` (stream-extracts per-cluster pre-initialized DBs to `/mydata/leveldb_twitter_cluster<N>_db` and trace files to `/mydata/twitter-traces`). The harness mirrors `lc-eval/ycsb/` with the same data-collection/benchmarking split (see `lc-eval/twitter/README.md`). **Benchmarking**: `lc-eval/twitter/reproduce_eval.sh [--model-dir <dir>] [--clusters "17 18 24 34 52"] [--factors "10 20 30 40"]` is the one-command entrypoint — it sequences `run_model_eval.sh` (5 classical policies + `cache_ext_fifo_ml_protect` with a **per-cluster matched model** `<model-dir>/twitter_cluster<N>_bench/model_weights.json` (the dir is keyed by benchmark name, like ycsb's `ycsb_a/`), default `--model-dir /mydata/models-jun-11`; plus Linux-LRU and kernel-MGLRU baselines) and `run_ml_sampling_eval.sh` looped per oversampling factor. `fifo_lc` and the BPF-mglru reimpl are dropped from the eval; the scripts **hard-fail if any cluster's model is missing**. Outputs (consolidated across clusters, all in `results/`): `twitter_eval_results.json`, `twitter_eval_{lru,mglru}.json`, `twitter_eval_ml{10,20,30,40}.json`, `twitter_eval_meta.json`, `twitter_eval_manifest.json`. **Tracer training data**: `lc-eval/twitter/collect_traces.sh` → `/mydata/cache_ext_logs/twitter_cluster<N>_bench/iter_<I>/` (+ `results/twitter_results_tracer.json`). Each eval script loops clusters (per-cluster DBs) but accumulates into one file per pass via the bench framework's default `--results-file` reuse. Driven by `lc-bench/bench_twitter_trace.py` (mirrors `bench_leveldb.py`; key differences: cgroup sized per cluster as `--cgroup-size-pct` (default 10%) of DB size +20 MiB, min 70 MiB — no `--cgroup-memory`; trace file `cat`-preloaded outside the cgroup). Twitter scripts build My-YCSB's `leveldb-latency` branch (latency tracking disabled — its arrays would dwarf the tiny cgroups), so Twitter results are **throughput-only**; the ycsb scripts switch the submodule back to `master`, and all branch switches use `git checkout -f` because the harness edits config YAMLs in-tree. `visualizations/results.ipynb` also consumes the `twitter_eval_*.json` files (throughput-only figures, guarded to skip until present).
