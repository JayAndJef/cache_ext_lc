...
- **YCSB+LevelDB (primary benchmark)**: `lc-eval/ycsb/reproduce_eval.sh <db>` is the one-command entrypoint — it produces every result file deterministically (`ycsb_eval_results.json` = 5 classical policies; `ycsb_eval_protect.json` = `ml_protect` matched models; `ycsb_eval_ml{10,20,30,40}.json` = one ML-rank oversampling factor each; `ycsb_eval_lru.json` = Linux classic LRU baseline / MGLRU off; `ycsb_eval_mglru.json` = kernel MGLRU baseline / MGLRU on; `ycsb_eval_meta.json` model mapping; `ycsb_eval_manifest.json` provenance). Mirrors the Twitter harness's split along *needs a model or not*: it sequences `run_ml_sampling_eval.sh` (the model half: `ml_protect` matched per-workload models → `ycsb_eval_protect.json`, plus the `cache_ext_ml_sampling` oversampling sweep — one file per factor, looped internally since checkpointing can't see loader-only args like `--sample_size`, plus the `ycsb_eval_meta.json` model map) and `run_heuristic_eval.sh` (the no-model half: 5 classical cache_ext policies + Linux-LRU and kernel-MGLRU baselines). YCSB uses a fixed 10G cgroup (no per-workload `cgroup_sizes.sh` table, unlike Twitter). `collect_traces.sh <db>` collects `fifo_lc` tracer training data (binary logs to `/mydata/cache_ext_logs/<benchmark>/iter_<N>/`, three `mglru_lc_{access,insertion,eviction}_{unix_ts}.bin` per dir). `collect_evict_latency.sh` captures raw per-call eviction-decision latency to `results/evict_latency/<tag>.trace`. **`visualizations/results.ipynb` is the single consumer** of all the result/`.trace` files — there are no summarize/validation scripts. See `lc-eval/ycsb/README.md` for the full pipeline. (The former generic `run.sh` full-sweep was removed; nothing depended on it.)
- **Twitter traces (paper Fig 8, clusters 17/18/24/34/52)**: artifacts via `./download_twitter_dbs.sh` (stream-extracts per-cluster pre-initialized DBs to `/mydata/leveldb_twitter_cluster<N>_db` and trace files to `/mydata/twitter-traces`). The harness mirrors `lc-eval/ycsb/` with the same data-collection/benchmarking split (see `lc-eval/twitter/README.md`). **Benchmarking**: `lc-eval/twitter/reproduce_eval.sh [--model-dir <dir>] [--clusters "17 18 24 34 52"] [--factors "10 20 30 40"]` is the one-command entrypoint — it sequences `run_heuristic_eval.sh` (the no-model half: 5 classical cache_ext policies + Linux-LRU and kernel-MGLRU baselines) and `run_ml_sampling_eval.sh` (the model half: `cache_ext_fifo_ml_protect` with a **per-cluster matched model** `<model-dir>/twitter_cluster<N>_bench/model_weights.json` (the dir is keyed by benchmark name, like ycsb's `ycsb_a/`), default `--model-dir /mydata/models-jun-11`, plus the `cache_ext_ml_sampling` oversampling-factor sweep — one file per factor, looped internally). The split is along *needs a model or not*: `run_heuristic_eval.sh` takes no `--model-dir`, the model half **hard-fails if any cluster's model is missing**. `fifo_lc` and the BPF-mglru reimpl are dropped from the eval. Outputs (consolidated across clusters, all in `results/`): `twitter_eval_results.json` (5 classical), `twitter_eval_protect.json` (ml_protect), `twitter_eval_{lru,mglru}.json`, `twitter_eval_ml{10,20,30,40}.json`, `twitter_eval_meta.json`, `twitter_eval_manifest.json`. **Tracer training data**: `lc-eval/twitter/collect_traces.sh` → `/mydata/cache_ext_logs/twitter_cluster<N>_bench/iter_<I>/` (+ `results/twitter_results_tracer.json`). Each eval script loops clusters (per-cluster DBs) but accumulates into one file per pass via the bench framework's default `--results-file` reuse. Driven by `lc-bench/bench_twitter_trace.py` (mirrors `bench_leveldb.py`; key differences: cgroup sized per cluster from `lc-eval/twitter/cgroup_sizes.sh` — a shared `CGROUP_BY_CLUSTER` table of `"<size-pct> <floor-mib>"` pairs sourced by all four Twitter scripts (collect_traces.sh, run_heuristic_eval.sh, run_ml_sampling_eval.sh, reproduce_eval.sh), so there are **no** `--cgroup-size-pct`/`--cgroup-floor-mib` CLI flags anymore (edit the table; an unsized cluster hard-errors). The limit is size-pct of DB size +20 MiB, floored at floor-mib — no `--cgroup-memory`; trace file `cat`-preloaded outside the cgroup. The original uniform 10%/70 MiB floored clusters 17/18/24 into a reclaim-thrash regime that livelocked the ML sampler and exposed a kernel sampling-eviction UAF, so the table defaults to 15%/192 MiB, with per-cluster floors so small clusters (e.g. 18, ~151 MiB DB) can be tuned below the floor to see real eviction pressure. One table for collection and eval keeps train/serve parity). Twitter scripts build My-YCSB's `leveldb-latency` branch (latency tracking disabled — its arrays would dwarf the tiny cgroups), so Twitter results are **throughput-only**; the ycsb scripts switch the submodule back to `master`, and all branch switches use `git checkout -f` because the harness edits config YAMLs in-tree. `visualizations/results.ipynb` also consumes the `twitter_eval_*.json` files (throughput-only figures, guarded to skip until present).
...
- **Tracer binary log format**: each policy writes `struct cache_access_fields` records to the `.bin` logs; that struct layout must stay in sync between the policy's `.bpf.c` (writer) and `.c` (loader) — if you change one, change the other.
- **Run YCSB bench directly**: `python3 lc-bench/bench_leveldb.py --policy-loader policies/<policy>.out --leveldb-db <db> --bench-binary-dir My-YCSB/build --benchmark ycsb_a,... [--model-file <json>] [--cgroup-memory 10G]`. Pass `--model-file` only with `cache_ext_fifo_ml_protect.out` or `cache_ext_ml_sampling.out` (the `CacheExtPolicy` forwards it to the loader); `run_ml_sampling_eval.sh` wires the matched per-workload models up automatically (it runs both `ml_protect` and the `ml_sampling` sweep).
...
- **Disk budget:** ~1.6–2.7 GiB of logs per workload-iteration — check
  ...
  insertion,eviction}_*.bin` per dir).
