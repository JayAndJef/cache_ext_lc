...
#include "cache_ext_fifo_ml_protect.skel.h"
...
char *USAGE =
	"Usage: ./cache_ext_fifo_ml_protect --watch_dir <dir> --cgroup_path <path> --model_file <json>\n";
...
// Must match struct model_meta in cache_ext_fifo_ml_protect.bpf.c.
struct model_meta {
	int64_t bias;
	int64_t threshold;
};
...
static int load_model_weights(const char *model_file,
			      struct cache_ext_fifo_ml_protect_bpf *skel)
{
	...
	// Bias + decision threshold (quantized).
...
int main(int argc, char **argv)
{
	...
	skel = cache_ext_fifo_ml_protect_bpf__open();
	if (skel == NULL) {
		perror("Failed to open BPF skeleton");
		goto cleanup;
	}

	printf("Starting cache_ext_fifo_ml_protect (skip-in-place reuse classifier)\n");

	skel->rodata->watch_dir_path_len = strlen(watch_dir_full_path);
	strcpy(skel->rodata->watch_dir_path, watch_dir_full_path);

	ret = cache_ext_fifo_ml_protect_bpf__load(skel);
	if (ret) {
	...
	link = bpf_map__attach_cache_ext_ops(skel->maps.protect_ops, cgroup_fd);
	if (link == NULL) {
		perror("Failed to attach BPF cache_ext_ops to cgroup");
		goto cleanup;
	}

	ret = cache_ext_fifo_ml_protect_bpf__attach(skel);
	if (ret) {
...
cleanup:
	...
	cache_ext_fifo_ml_protect_bpf__destroy(skel);
	return ret;
}
