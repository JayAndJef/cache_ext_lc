...
#include "cache_ext_mglru_lc_revert.skel.h"
#include "dir_watcher.h"

char *USAGE = "Usage: ./cache_ext_mglru_lc_revert --watch_dir <dir> --cgroup_path <path> [--log_dir <dir>]\n";
struct cmdline_args {
	char *watch_dir;
	char *cgroup_path;
	char *log_dir;
...
{
	int ret = 1;
	struct cache_ext_mglru_lc_revert_bpf *skel = NULL;
	...
	// Open skel
	skel = cache_ext_mglru_lc_revert_bpf__open();
	if (skel == NULL) {
	...
	// Load programs
	ret = cache_ext_mglru_lc_revert_bpf__load(skel);
	if (ret) {
	...
	// Attach probes
	ret = cache_ext_mglru_lc_revert_bpf__attach(skel);
	if (ret) {
...
cleanup:
    ...
    cache_ext_mglru_lc_revert_bpf__destroy(skel);
    return ret;
}
