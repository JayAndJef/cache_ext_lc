...
class MGLRULCRevertBenchmark(BenchmarkFramework):
	def __init__(self, benchresults_cls=BenchResults, cli_args=None):
		super().__init__("mglru_lc_revert_benchmark", benchresults_cls, cli_args)
	...
	def generate_configs(self, configs: List[Dict]) -> List[Dict]:
		...
		configs = add_config_option(
			"cgroup_size", [parse_memory_string(self.args.cgroup_memory)], configs
		)
		...
		configs = add_config_option("benchmark", ["mglru_lc_revert"], configs)
		configs = add_config_option(
...
def main():
	...
	global log
	mglru_lc_revert_bench = MGLRULCRevertBenchmark()
	if not os.path.exists(mglru_lc_revert_bench.args.watch_dir):
		raise Exception(
			"Watch directory not found: %s" % mglru_lc_revert_bench.args.watch_dir
		)
	log.info("Watch directory: %s", mglru_lc_revert_bench.args.watch_dir)
	mglru_lc_revert_bench.benchmark()
