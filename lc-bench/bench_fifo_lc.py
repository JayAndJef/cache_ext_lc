...
class FIFOLCBenchmark(BenchmarkFramework):
    def __init__(self, benchresults_cls=BenchResults, cli_args=None):
        super().__init__("fifo_lc_benchmark", benchresults_cls, cli_args)
...
def main():
    ...
    fifo_lc_bench = FIFOLCBenchmark()
    # Check that watch dir exists
    if not os.path.exists(fifo_lc_bench.args.watch_dir):
        raise Exception(
            "Watch directory not found: %s" % fifo_lc_bench.args.watch_dir
        )
    log.info("Watch directory: %s", fifo_lc_bench.args.watch_dir)
    fifo_lc_bench.benchmark()
